...
## script by reviewing/running testCacheMatrix.R in this same repository:
## https://github.com/reggiemack-310/ProgrammingAssignment2
## 
## makeCacheMatrix:
##     
##     This function initializes the current environment with the matrix to be
...
##         "cache"
##         
##         Thus the list of 4 named functions generated by nd returned from the 
...
## cacheSolve:
## 
##     This function either computes the inverse of an invertible matrix or 
##     retrieves the precomputed inverse from "cache". Passed to this function 
...
##         matrix cached is the same as that passed then return the retrieved 
##         inverse along with a message indicating said retrieval. Otherwise, 
##         cache the new matrix to be inverted, then compute the inverse, cache
...
##     generated indicating "getting cached matrix ..."
## 
##     when the inverse is required of another invertible matrix then the new
##     matrix's is cached, its inverse calculated and "cached" and its inverse 
##     returned.
##     
...
## 
##     1. Initialize 
##        x <- makeCacheMatrix(m)      # Assign the list of named function to x
## 
...
##                                     # from cache
## 
##     4. Compute the inverse of mInv1 which is the inverse of m and assign to
##        mInv3
##        mInv3 <- cacheSolve(x,mInv2) # now the inverse of mInv2 is computed 
##                                     # and assigned to mInv3
## 
##     Now m and mInv3 should be identical matrices ...
## 
##-----------------------------------------------------------------------------
...
##     manipulate the caching of an inverse of the matrix
##     passed, with setter & getter functions.
##
...
makeCacheMatrix <- function(x = matrix()) {  # beg function
  ...
  getsolve <- function() m                   # define getsolve funtc
  list(                                      # create a list of 4 elements where each elememnt is 
    set = set, get = get,                    #        is 1 of the 4 functions define just prior
    setsolve = setsolve,
    getsolve = getsolve
  )                                          # return the list
...
##     argument passed to it.  It returns either the calculated inverse 
##     of the matrix  passed or if the inverse of the matrix to be 
##     calculated has been cached (i.e., the same or identical to the 
##     matrix cached) then the pre-calculated matrix inverse is returned 
##     from cache.
##
##     parameters passed to this function:
##     X -> vector generated by makeCacheVector
##     y -> "invertible" matrix to be inverted
##     ...  additional parameter(s) passed on through to the solve() function
##          when needed. (see ?solve)
## 
##-----------------------------------------------------------------------------
...
cacheSolve <- function(x, y=matrix(), ...) { # beg function
  ...
  if(!is.null(m)&&identical(y,x$get())) {    # if cached & identical to passed matrix
    message("getting cached matrix ...")     #    indicate so by issuing a message
    return(m)                                #    exit returning the cached inverse
  } else {                                   # otherwise
    x$set(y)                                 #    cache the new invertible matrix
  }                                          # end if
  data <- x$get()                            # retrieve the invertible matrix
